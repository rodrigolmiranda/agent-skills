...
Delivery manager owns outcome, dependencies, lead selection and final accountability. Coordination does not itself grant judgment authority. Sol high is the normal brain for product and architecture decisions; Astra medium handles unusually large cross-workstream ambiguity. Fable 5.1 medium is a product-ideation brain when its included-plan/no-extra-cost preflight passes. An Opus 5 high manager remains an approved continuity option when Codex capacity is unavailable, but routes unresolved interpretation to an approved brain. Add leads for independently manageable workstreams, including workstreams spanning several repositories. Do not equate one repository with one lead or one model with one role. Manager spends its context on decisions, exception summaries and integration evidence.
...
Separate judgment from mechanical coordination before selecting a lead. Sol high resolves ordinary product, architecture and consequential tradeoffs. Astra medium resolves unusually large ambiguity across workstreams. Fable 5.1 medium explores product ideas and alternatives without implementing them. Terra leads, lower-effort Sol, Opus workers and other execution profiles may coordinate only settled contracts; they return unresolved choices to a brain. Size alone is not a risk measure.
