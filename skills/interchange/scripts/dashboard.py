...
def task_card(item, attempts):
    ...
    title = item.get('title') or (current.get('task') if current else None) or item.get('id')
    ...
    activity_state = ((current.get('execution_state') or current.get('last_observed_state'))
                      if current and (current.get('execution_state') or current.get('last_observed_state'))
                      else item.get('state'))
    ...
    content = ('<article class="task-card"><div class="card-title"><strong>' + esc(title)
               + '</strong><span class="badge ' + status_class(activity_state) + '">'
               + 'Snapshot status: ' + esc(status_label(activity_state)) + '</span></div>'
...
def board(record):
    ...
    for key, label in (('working', 'Working now'), ('blocked', 'Blocked'), ('next', 'Next'), ('done', 'Done')):
        entries = columns[key]
