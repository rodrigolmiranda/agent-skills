...
Prefer the Builder for roughly 90% of execution, not for unresolved product decisions. Money, concurrency, idempotency, tenancy/RLS and cross-repository contract slices are the exception: route them to a Senior Builder from the start, or after the second CHANGES-NEEDED on the same surface ([evidence](packets-and-review-loop.md#routing-by-risk-surface)). A senior is justified by ambiguity/consequence left *within* an accepted contract, not by file count. Architecture/security review is exceptional and early when it can change a design economically.
