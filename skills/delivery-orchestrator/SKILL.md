...
At takeover refresh current heads, ownership and changed dependency evidence before dispatch. Re-run the scheduling checkpoint after every dispatch/delivery, worker return/question/failure, review disposition, unblock, ownership release or scope change, and before yielding. At every wake apply the [progress and repeated-wait checkpoint](references/scheduling.md#progress-checkpoint-and-repeated-waits); separate writing reservations from acceptance waits. Reuse unchanged evidence rather than re-reading every document. Record an incomplete source scan as incomplete, never as “nothing else can run.”
