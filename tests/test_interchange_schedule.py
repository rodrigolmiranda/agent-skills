...
def setUpModule():
    global EVIDENCE_DIR, EVIDENCE_ROOT
    EVIDENCE_DIR = tempfile.TemporaryDirectory()
    EVIDENCE_ROOT = Path(EVIDENCE_DIR.name)
    for name in ('capacity/snapshot-17', 'conflict/source', 'dispatch/startup',
                 'inventory/parents-and-children', 'inventory/query', 'packet/request',
                 'recovery/alternatives', 'recovery/current-cause', 'recovery/full-backlog',
                 'schedule/current', 'supervisor/request-owner', 'worker/one-started',
                 'writer/exit', 'writer/timeout'):
...
def validate(record, **kwargs):
    return check_schedule.validate_schedule(record, artifact_root=EVIDENCE_ROOT, **kwargs)
...
class ScheduleReceiptTests(unittest.TestCase):
    def test_complete_parallel_dispatch_is_consistent(self):
        ...
        self.assertEqual([], validate(receipt(items)))
    ...
    def test_duplicate_item_ids_are_rejected(self):
        ...
        errors = validate(receipt([first, duplicate], ['PROJECT#1']))
    ...
    def test_unknown_disposition_is_rejected_without_crashing_on_non_string(self):
        ...
        errors = validate(receipt([item], ['PROJECT#1']))
    ...
    def test_ready_idle_requires_evidence_backed_allowed_exclusion(self):
        ...
        errors = validate(receipt([placeholder_evidence], ['PROJECT#1']))
        self.assertTrue(any('.evidence must be a concrete pointer for held work' in error
                            for error in errors))

        malformed_evidence = dict(placeholder_evidence, evidence='https://[')
        errors = validate(receipt([malformed_evidence], ['PROJECT#1']))
        self.assertTrue(any('.evidence must be a concrete pointer for held work' in error
                            for error in errors))
        ...
        errors = validate(receipt([coordinator_action], ['PROJECT#1']))
    ...
    def test_measured_capacity_exclusion_with_evidence_is_consistent(self):
        ...
        self.assertEqual([], validate(receipt([item], ['PROJECT#1'])))
    ...
    def test_coordinator_action_needs_a_concrete_evidence_backed_next_check(self):
        ...
        errors = validate(receipt([unverified], ['PROJECT#1']))
        ...
        self.assertEqual([], validate(receipt([verified], ['PROJECT#1'])))
    ...
    def test_cli_labels_success_as_consistency_only(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', suffix='.json') as receipt_file:
            ...
            result = subprocess.run(
                [sys.executable, str(script), receipt_file.name, "--artifact-root", str(EVIDENCE_ROOT)],
...
class ProgressCheckpointTests(unittest.TestCase):
    def test_old_inventory_alone_cannot_certify_progress(self):
        ...
        self.assertTrue(any('checkpoint is required' in e for e in validate(record)))

    def test_two_unchanged_checks_require_recovery_across_all_work(self):
        record = receipt()
        record['checkpoint']['unchanged_checks'] = 2
        self.assertTrue(any('recovery is required' in e for e in validate(record)))
        ...
        self.assertTrue(any('next_check_at' in e for e in validate(record)))
    ...
    def test_inventory_needs_children_and_retrieval_provenance(self):
        ...
        self.assertTrue(any('hierarchy_evidence' in e for e in validate(record)))
        self.assertTrue(any('source_retrieved_at' in e for e in validate(record)))
    ...
    def test_stopped_writer_does_not_reserve_acceptance_surface(self):
        ...
        self.assertTrue(any('stopped writer' in e for e in validate(record)))
    ...
    def test_unknown_writer_requires_recheck_not_automatic_release(self):
        ...
        self.assertTrue(any('never auto-release' in e for e in validate(record)))
    ...
    def test_released_lease_cannot_block_a_ready_item(self):
        ...
        self.assertTrue(any('released or absent' in e for e in validate(receipt([item]))))
    ...
    def test_yield_cannot_leave_an_unowned_readiness_action(self):
        ...
        self.assertTrue(any('executable coordinator-action' in e for e in validate(record)))
    ...
    def test_exit_validation_applies_to_reused_worker_return_receipt(self):
        ...
        self.assertTrue(any('executable coordinator-action' in e for e in
                            validate(record, before_yield=True)))
        script = Path(check_schedule.__file__)
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json') as f:
            json.dump(record, f)
            f.flush()
            result = subprocess.run([sys.executable, str(script), f.name, '--before-yield', '--artifact-root', str(EVIDENCE_ROOT)],
...
class LocalEvidenceTests(unittest.TestCase):
    def test_file_backlog_without_github_is_valid(self):
        record = receipt()
        record.update(source_kind='files', scope_url='artifact://inventory/query')
        self.assertEqual([], validate(record))

    def test_missing_artifact_is_rejected(self):
        record = receipt()
        record['checkpoint']['assessment_evidence'] = 'artifact://missing/report.md'
        self.assertTrue(any('does not exist' in e for e in validate(record)))

    def test_relative_and_file_uri_evidence_resolve(self):
        record = receipt()
        record['checkpoint']['assessment_evidence'] = 'schedule/current'
        record['checkpoint']['source_query'] = (EVIDENCE_ROOT / 'inventory/query').as_uri()
        self.assertEqual([], validate(record))

    def test_artifact_escape_and_directory_are_rejected(self):
        record = receipt()
        for value in ('artifact://../outside', 'artifact://inventory/'):
            record['checkpoint']['assessment_evidence'] = value
            self.assertTrue(validate(record))

    def test_unknown_backlog_type_rejected(self):
        record = receipt()
        record['source_kind'] = 'imaginary'
        self.assertTrue(any('source_kind' in e for e in validate(record)))
